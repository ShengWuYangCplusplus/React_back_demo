...
const IconFont = createFromIconfontCN({
  scriptUrl: [
    '//at.alicdn.com/t/font_1296985_2fkg7vd3vkv.js'
  ],
});
...
class CustomMenu extends React.Component {
  ...
  renderMenuItem = ({key, icon, title,}) => {
    return (
    <Menu.Item key={key} icon={<IconFont type={icon} />}>
        <Link to={key}>
          <span>{title}</span>
        </Link>
      </Menu.Item>
  ...
  renderSubMenu = ({key, icon, title, subs}) => {
    return (
    <Menu.SubMenu key={key} icon={<IconFont type={icon} />} title={<span>{title}</span>}>
        {
          subs && subs.map(item => {
