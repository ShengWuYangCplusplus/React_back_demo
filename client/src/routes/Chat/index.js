...
import { initChatList, initWebSocket, getUser } from "src/store/actions";
import { connect } from "react-redux";
import { bindActionCreators } from "redux";
import { isAuthenticated } from "src/utils/Session";
import { apis } from "src/request/apis";
import { replaceImg, throttle } from "src/utils/util";
...
const store = connect(
  ...
  (dispatch) =>
    bindActionCreators({ getUser, initChatList, initWebSocket }, dispatch)
);
...
class Chat extends Component {
  ...
  isAllowNotify() {
    ...
    if (window.Notification && Notification.permission !== "denied") {
      Notification.requestPermission(function (status) {
        if (status === "granted") {
          _this.setState({ allowNotify: true });
  ...
  setNotification() {
    var notify = new Notification("新消息", {
      body: "xxx发来了新消息",
      lang: "zh-CN",
      icon:
        "http://backend.jin6.com/@webroot/uploads/image/20180423/1524455833949078.png",
    });
  ...
  getUserList = async () => {
    ...
    //     userList: res.data || []
    // })
    apis.System.getUserList({ index: 0, size: 10 }).then((res) => {
      console.log(res);
      this.setState({
  ...
  handleUserList = () => {
    ...
    for (let item of userList) {
      const isHave = onlineList.find((i) => i.id === item._id);
      const user = {
        ...item,
        online: !!isHave,
      };
        ...
        <div className="chat-box" ref={(el) => (this.chatBox = el)}>
          ...
          <div className="chat-body">
            <div className="left">
              ...
              {this.props.user.friends.map((item, idx) => {
                return (
                  <div className="left-item" key={idx}>
                    <div>
                    ...
                    <div className="left-item-text">
                      <div className="group-name">
                        ...
                        <span>
                          {
                            this.handleTime(lastChat.createTime, true).split(
                              " "
                            )[0]
                          }
                        </span>
            ...
            <div className="right">
              ...
              <List
                ...
                rowRenderer={({ key, index, style }) => (
                  <div key={key} className="user-item" style={{...style,cursor:'pointer'}}  onClick={
                    this.showFriend.bind(this,userList[index])
                }>
                    <div
                    ...
                    <div
                      className={`ellipsis ${
                        userList[index].online ? "isOnline" : "offLine"
                      }`}
                    ...
                    <div
                      style={{
                        display:
                          userList[index].roleId == "1" ? "block" : "none",
                      }}
                    >
                      <img
                        width={18}
